//
// ECHOCLNT.C	データグラムECHOクライアント
//
//				WS2_32.DLLに動的にリンクするか、
//				WinSock 2が使用不可能な時には
//				WSOCK32.DLLへ動的にリンクします。
//
//				WinSock 2では特定のプロトコルに依存せず、
//				WinSock 1.1ではUDPを使用します。
//
//				WS2_32.LIBにもWSOCK32.LIBにも直接リンクしないこと。
//
...
void FillSocketChoices(HWND hwnd)
{
	...
	pSock      = (SOCKET *)gpSockets;
	gNbrSockets= nRet;
	nTempNbrSockets = nRet;
	for(nCount = 0; nCount < nTempNbrSockets; nCount++)
	{
...
void RecvEcho(HWND hwnd, SOCKET socket, int nErrorCode)
{
	...
	int			nCount;
	int			nRet;
	static BOOL bInsideError = FALSE;
